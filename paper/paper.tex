...
\multirow{8}{*}{\textbf{Cocoa}}
  ...
  & SV-L       & 57  & 4123 & 0.014 & F & F & P & P & F & P \\
...
\multirow{8}{*}{\textbf{Oil}}
  ...
  & SV-$t$     & 75  & 4806 & 0.016 & F & F & P & P & F & P \\
  & SV-L       & 71  & 4806 & 0.015 & F & P & F & P & P & P \\
  & \textbf{SV-$t$L}    & \textbf{59}  & 4806 & 0.012 & P & F & P & P & P & P \\
\bottomrule
\end{tabular}
\vspace{4pt}
...
\textbf{SV-$t$} & \textbf{6} & \textbf{6} & 3 & 15 \\
SV-Leverage     & 3 & 4 & 3 & 10 \\
...
Oil is where this study's results depart most sharply from a tidy narrative, and we report this plainly rather than around it. Every model we estimate except one fails the 99\% VaR Kupiec test for oil. GARCH produces 80 violations against an expected 48, EGARCH 86, the OU benchmark 62 (on 4,560 converged windows, and the only benchmark-commodity combination in this entire study to also fail at 95\% VaR, on both Kupiec and Christoffersen), and Historical Simulation 74. Among the primary models, SV-Gaussian produces 78 violations and SV-$t$ 75 --- both higher than the benchmarks' typical range, and SV-$t$ additionally fails the Christoffersen test at 95\% VaR, indicating that the Student-$t$ innovation that resolved 99\% VaR adequacy for both cocoa and gold does not do so for oil at the same confidence level --- while SV-Leverage produces 71 violations and fails all three 99\% tests outright, the only model-commodity combination in this study to do so.

SV-$t$-Leverage is the exception, and the way it is an exception is itself informative. With 59 violations against 48 expected --- the smallest overshoot among any oil model sharing the same $T=4806$ observation count --- it \emph{passes} the Kupiec test at 99\% ($p=0.126$): the overall frequency of extreme losses is correctly calibrated. What fails is the Christoffersen conditional coverage test ($p=0.039$), driven specifically by its independence component ($p=0.042$): violations are correctly frequent in aggregate, but they cluster in time rather than occurring independently. This is a materially different and, we think, more practically consequential failure mode than a simple frequency miscalibration. A risk model whose tail losses are correctly rare on average but arrive in clusters is failing exactly when clustering matters most --- during the crisis episodes, such as the COVID-19 collapse or the 2026 Middle East-driven shock, that this paper's own crisis-period analysis in Section~\ref{sec:stylised} identifies as the source of oil's extreme kurtosis in the first place.
